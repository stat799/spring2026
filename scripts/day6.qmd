...
m <- lmer(force ~ fat_perc+ Temperature + fat_perc:Temperature +
            (1|Day) + (1|Day:Temperature),
          data = d1)
getME(m, "X")
getME(m, "Z")
getME(m, "b")
...
library(emmeans)
emmeans(m, ~fat_perc:Temperature)
emmeans(m, ~Temperature, contr = list(c(1, -1, 0)))
emmeans(m, ~fat_perc, contr = list(c(1, -1, 0, 0)))
```

## Applied example II -- zero VC

Designed experiment studying the effect of 4 treatments on gut microorganisms. Colony forming units were studied before and after 4 interventions. Patient was the EU, and all patients were handled the same.
...
m2 <- lmer(logCFU ~ Trt*Time + (1|patient_id), data = d2)
...
```{r}
m2.1 <- lm(logCFU ~ Trt*Time, data = d2)
DHARMa::simulateResiduals(m2.1, plot = T)
...
m3.1 <- lmer(yield ~ trt + (1|block), data = d7)
m3.2 <- lm(yield ~ trt + block, data = d7)
...
A study of gastro-intestinal parasites in dairy cows The outcome of interest is fecal egg count/5 gram of feces (fec) (see second figure for summary statistics for fec). Independent variables include lact (lactation; multiparous (1)/primiparous (0)), manure (manure spread on pasture; yes(1)/no(0)).
