...
### Applied example II -- convergence issue I 
...
```{r}
m2.f <- lm(logCFU ~ Trt*Time + patient_id, data = d2)
sigma(m2.f)
```
...
A study of gastro-intestinal parasites in dairy cows based on a longitudinal study. 
The outcome of interest is fecal egg count/5 gram of feces (fec) (see second figure for summary statistics for fec).
Independent variables include lact (lactation; multiparous (1)/primiparous (0)), man_heif (manure spread on heifer pasture; yes(1)/no(0)), and man_lact (manure spread on cow pasture; yes(1)/no(0)).
